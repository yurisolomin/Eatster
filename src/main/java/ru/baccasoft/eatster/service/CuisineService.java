...
public class CuisineService {
    ...
    private static final String SQL_TABLE = "cuisine";
    ...
    private static final CuisineMapper MAPPER = new CuisineMapper();
    ...
    private static final class CuisineMapper implements RowMapper<CuisineModel> {
        @Override
        public CuisineModel mapRow(ResultSet rs, int rowNum) throws SQLException {
            CuisineModel item = new CuisineModel();
            int index = 0;
    ...
    public List<CuisineModel> findAll() {
        LOG.debug("findAll:");
        List<CuisineModel> list = jdbc.query(SQL_SELECT_ALL, MAPPER);	
        LOG.debug("Ok. return items {0}",list.size());
        return list;
    }
    
    @Transactional(readOnly=true)
    public CuisineModel getItem(long id) {
        LOG.debug("getItem: id={0}",id);
        List<CuisineModel> list = jdbc.query(SQL_SELECT_ITEM, new Object[]{id}, MAPPER );
        if (list.size() == 1) {
    ...
    @Transactional
    public long insertItem(final CuisineModel item) {
    ...
    @Transactional
    public int updateItem(CuisineModel item) {
    ...
    @Transactional(readOnly=true)
    public List<CuisineModel> findChanged(Timestamp timestamp) {
        LOG.debug("findChanged: timestamp={0}",timestamp);
        List<CuisineModel> list = jdbc.query(SQL_SELECT_CHANGED, new Object[]{timestamp}, MAPPER);		
        LOG.debug("Ok. return items {0}",list.size());
        return list;
    }
