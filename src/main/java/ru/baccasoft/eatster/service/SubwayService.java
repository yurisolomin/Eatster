...
public class SubwayService {
    private static final Logger LOG = Logger.getLogger(SubwayService.class);
    
    private JdbcTemplate jdbc;
    private static final String SQL_TABLE = "subway";
...
*/    
    ...
    private static final SubwayMapper MAPPER = new SubwayMapper();
    ...
    private static final class SubwayMapper implements RowMapper<SubwayModel> {
        @Override
        public SubwayModel mapRow(ResultSet rs, int rowNum) throws SQLException {
            SubwayModel item = new SubwayModel();
            int index = 0;
    ...
    public SubwayModel getItem(long id) {
        LOG.debug("getItem: id={0}",id);
        List<SubwayModel> list = jdbc.query(SQL_SELECT_ITEM, new Object[]{id}, MAPPER );
        if (list.size() == 1) {
    ...
    @Transactional(readOnly=true)
    public List<SubwayModel> findAll() {
        LOG.debug("findAll:");
        List<SubwayModel> list = jdbc.query(SQL_SELECT_ALL, MAPPER);	
...
/*    
    ...
    @Transactional
    public long insertItem(final SubwayModel item) {
    ...
    @Transactional
    public int updateItem(SubwayModel item) {
...
*/
    @Transactional(readOnly=true)
    public List<SubwayModel> findChanged(Timestamp timestamp) {
        LOG.debug("findChanged: timestamp={0}",timestamp);
        List<SubwayModel> list = jdbc.query(SQL_SELECT_CHANGED, new Object[]{timestamp}, MAPPER);		
        LOG.debug("Ok. return items {0}",list.size());
        return list;
    }
